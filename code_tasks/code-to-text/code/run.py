            ...
            if args.do_eval:
                ...
                for lang in LANGUAGES:
                    ...
                    if 'dev_loss' in dev_dataset:
                        ...
                        eval_examples = read_examples(os.path.join(os.path.dirname(args.train_data_file), lang, 'valid.jsonl'))
                    ...
                    for key in sorted(result.keys()):
                        logger.info("[%s] --  %s = %s", lang, key, round(result[key], 4))
                    logger.info("  " + "*" * 20)

                total_eval_loss /= len(LANGUAGES)
                ...
                if total_eval_loss < best_loss:
                    logger.info("  Best ppl:%s", round(np.exp(total_eval_loss), 5))
                    logger.info("  " + "*" * 20)
